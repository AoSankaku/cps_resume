...
const getHeroInfo = (nickname) => {
	for (let i = 0, l = heroes.heroes.length; i < l; i++) {
		console.log(heroes.heroes[i].nickname);
		if (nickname == heroes.heroes[i].nickname) return heroes.heroes[i];
	}
	return undefined;
}
...
//画像をプリロード
let imgTempArray = new Array(13);
const img = imgTempArray.fill().map(e => (new Image()));
